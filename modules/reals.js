...
function WhyNotReal(s) {
    ...
    for (let i = 0; i < s.length; i++) {
        if (!allowed_chars.includes(s[i])) {
            return `The character "${s[i]}" is not allowed.`;
        }
    }
    s = s.split('.');
    if (s.length > 2) {
    ...
    for (let i = 0; i < intstr.length; i++) {
        if (!digits.includes(intstr[i])) {
            return `"+" and "-" don't belong inside a number.`;
        }
    }
    if (intstr.length > 1 && intstr[0] === '0') {
        return `Remove those leading zeroes.`;
    }
    if (s.length == 2) {
        let floatstr = s[1];
        for (let i = 0; i < floatstr.length; i++) {
            if (!digits.includes(floatstr[i])) {
                return `The character "${floatstr[i]}" is not allowed inside a number.`;
            }
        }
    }
    return "It it Real.";
...
// export {Real, isReal, WhyNotReal, RealListIncludes}
